...
type BoostrapKeyDistroStateMachine struct {
	...
	bootstrapFsmDA    *data_access.FsmDA
	db                *sql.DB
}
...
func (sm *BoostrapKeyDistroStateMachine) Transit() error {
	...
	for {
		var err error
		err = sm.bootstrapFsmDA.SetFSM(sm.StateMachineName, sm.RequestId, sm.CurrentState.StateName, false, sm.IsTraversalMode)
		...
		if err != nil {
			...
			_ = sm.bootstrapFsmDA.SetFSM(sm.StateMachineName, sm.RequestId, sm.CurrentState.StateName, true, sm.IsTraversalMode)
		}
		_ = sm.bootstrapFsmDA.SetFSM(sm.StateMachineName, sm.RequestId, sm.CurrentState.StateName, true, sm.IsTraversalMode)
...
func GenerateKeyDistroStateMachine(requestId int64, databse *sql.DB) BoostrapKeyDistroStateMachine {
	...
	generateMessageState := State{
		...
		Action: func(T any) error {
			...
			cacheHandler.SetRequestInformation(sm.RequestId, "key_distro_generatedMsg", b64.StdEncoding.EncodeToString(msgBytes))
			return nil
		},
		ReverseAction: func(T any) error {
			return nil
	...
	sendMessageToVerifierState := State{
		...
		Action: func(T any) error {
			...
			data, err := cacheHandler.GetRequestInformation(sm.RequestId, "key_distro_generatedMsg")
			if err == nil {
				...
				cacheHandler.SetRequestInformation(sm.RequestId, "key_distro_responseMsg", b64.StdEncoding.EncodeToString(responseBytes))
				return nil
			} else {
				sm.IsTraversalMode = false
				zap.L().Error("Error in sending message to verifier", zap.Error(err))
	...
	ParseAndApplyResponseState := State{
		...
		Action: func(T any) error {
			data, err := cacheHandler.GetRequestInformation(sm.RequestId, "key_distro_responseMsg")
			if err == nil {
